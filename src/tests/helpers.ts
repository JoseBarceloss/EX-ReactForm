...
export const superLongText = `${longText} ${longText}`;
...
export const testDataIsShowing = () => {
  const nameDisplayed = screen.getByText(expectedName, { ignore: 'input' });
  expect(nameDisplayed).toHaveTextContent(expectedName);

  const emailDisplayed = screen.getByText(expectedEmail, { ignore: 'input' });
  expect(emailDisplayed).toHaveTextContent(expectedEmail);

  const cpfDisplayed = screen.getByText(expectedCpf, { ignore: 'input' });
  expect(cpfDisplayed).toHaveTextContent(expectedCpf);

  const addressDisplayed = screen.getByText(expectedAddress, { ignore: 'input' });
  expect(addressDisplayed).toHaveTextContent(expectedAddress);

  const cityDisplayed = screen.getByText(expectedCity, { ignore: 'input' });
  expect(cityDisplayed).toHaveTextContent(expectedCity);

  const stateDisplayed = screen.getByText(expectedState, { ignore: 'option' });
  expect(stateDisplayed).toHaveTextContent(expectedState);

  const addressTypeDisplayed = screen.getByText(expectedAddressType, { ignore: 'label' });
  expect(addressTypeDisplayed).toHaveTextContent(expectedAddressType);

  const resumeDisplayed = screen.getByText(expectedResume, { ignore: 'textarea' });
  expect(resumeDisplayed).toHaveTextContent(expectedResume);

  const roleDisplayed = screen.getByText(expectedRole, { ignore: 'input' });
  expect(roleDisplayed).toHaveTextContent(expectedRole);

  const roleDescriptionDisplayed = screen.getByText(expectedRoleDescription, { ignore: 'textarea' });
  expect(roleDescriptionDisplayed).toHaveTextContent(expectedRoleDescription);
};

export const mockAlert = () => vi.spyOn(global, 'alert')
  .mockImplementation(() => { });
