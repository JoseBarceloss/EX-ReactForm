...
describe('2 - Crie outro `<fieldset>` para dados de endereço', () => {
  ...
  /* Endereço */
  it('Será verificado se existe um campo do tipo `text` associado a uma `label` com o texto `Endereço`', () => {
  ...
  /* Cidade */
  it('Será verificado se existe um campo do tipo `text` associado a uma `label` com o texto `Cidade`', () => {
    const cityInput = screen.getByLabelText('Cidade');

    expect(cityInput).toHaveAttribute('type', 'text');
  });

  it('Será verificado se o campo `Cidade` tem o tamanho máximo de 28 caracteres', () => {
    const cityInput = screen.getByLabelText('Cidade');
  ...
  it('Será verificado que, ao digitar no campo `Cidade`, se nome da cidade começar por número, o campo é limpo após perder o foco', () => {
    const cityInput = screen.getByLabelText('Cidade');
  ...
  /* Estado */
  it('Será verificado se existe um campo do tipo `select` associado a uma `label` com o texto `Estado`', () => {
  ...
  it('Será verificado que, ao clicar no campo `Estado`, é possível selecionar um estado', () => {
    const stateSelect = screen.getByLabelText('Estado');
  ...
  /* TIPO */
  it('Será verificado se existe dois campos do tipo `Radio Button`. O primeiro deve estar associado a uma `label` com o texto `Casa` e o segundo por uma label com o texto `Apartamento`', () => {
